...
const KEEP = new Set(['MinorGC', 'MajorGC', 'V8.GCScavenger', 'V8.GCFinalizeMC', 'V8.GCCompactor',
  'FireAnimationFrame', 'Paint', 'Layerize', 'Commit', 'UpdateLayoutTree', 'Layout', 'TimerFire',
  'FunctionCall', 'RunTask', 'ThreadControllerImpl::RunTask']);
...
async function measureMode(page, cdp, mode) {
  ...
  const onData = ({ value }) => {
    for (const e of value) {
      if (e.cat?.includes('blink.user_timing') || KEEP.has(e.name) || (e.dur && e.dur > 8000)) events.push(e);
    }
  };
  cdp.on('Tracing.dataCollected', onData);
  await cdp.send('Tracing.start', {
    transferMode: 'ReportEvents',
    traceConfig: { includedCategories: ['devtools.timeline', 'v8', 'disabled-by-default-v8.gc', 'blink.user_timing', 'toplevel'] }
  });
  ...
  const window = await page.evaluate(m => {
    ...
    return {
      ...
      levels: { bass: +avg('bass').toFixed(2), mid: +avg('mid').toFixed(2), high: +avg('high').toFixed(2),
...
function summarize({ mode, sync, events, frames, levels, collageLit }) {
  ...
  const onMain = anchor ? events.filter(e => e.ts && `${e.pid}:${e.tid}` === main).map(e => ({
    name: e.name,
  ...
  return {
    mode, levels, collageLit,
    samples: kept.length,
    p50: fps(at(0.5)), p05: fps(at(0.95)), min: fps(at(1)),
    slowPer1000: +(slow.length * 1000 / (kept.length || 1)).toFixed(1),
    gc: { events: gcs.length, totalMs: Math.round(gcs.reduce((t, e) => t + e.dur, 0)),
    ...
    slowFrames: slow.slice(0, 20).map(f => {
      ...
      return {
        dt: +f.dt.toFixed(1),
        overlapping: onMain.filter(e => e.start < end && e.start + e.dur > start && e.dur > 2)
...
try {
  ...
  const env = await page.evaluate(() => {
    ...
    return {
      ...
      backing: [canvas.width, canvas.height], css: [canvas.clientWidth, canvas.clientHeight],
      dpr: devicePixelRatio, focus: document.hasFocus()
    };
  });
  console.log('env', JSON.stringify({ ...env, wav: WAV, full: FULL, throttle: THROTTLE, dwell: DWELL }));
  if (/swiftshader/i.test(env.renderer)) console.warn('WARNING: software renderer; FPS is not indicative');
  ...
  for (const mode of MODES) {
    ...
    for (const f of s.slowFrames) {
      console.log(`  slow ${f.dt} ms -> ${f.overlapping.join(' | ') || 'no main-thread task over 2 ms (GPU or compositor?)'}`);
    }
  }
  ...
  await writeFile(join(OUT, name), JSON.stringify({ env, options: { MODES, DWELL, WAV, DPR, FULL, THROTTLE }, summaries, errors }, null, 2));
